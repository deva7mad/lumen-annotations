...
class RouteScanner
{
    ...
    public function parseClass($class)
    {
        ...
        if ($annotation = $this->reader->getClassAnnotation($reflectionClass, '\DevA7mad\Annotations\Annotations\Controller')) {
    ...
    public function parseController($class)
    {
        ...
        foreach ($classAnnotations as $annotation) {
            ...
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Controller) {
                $prefix = $annotation->prefix;
                $middleware = $annotation->middleware;
            }
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Middleware) {
                $middleware = $annotation->value;
            }

            // resource controller
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Resource) {
    ...
    protected function hasHttpMethodAnnotation($name, $methodAnnotations)
    {
        foreach ($methodAnnotations as $annotation) {
            ...
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Get) {
                $httpMethod = 'GET';
                break;
            }
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Post) {
                $httpMethod = 'POST';
                break;
            }
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Options) {
                $httpMethod = 'OPTIONS';
                break;
            }
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Put) {
                $httpMethod = 'PUT';
                break;
            }
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Patch) {
                $httpMethod = 'PATCH';
                break;
            }
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Delete) {
                $httpMethod = 'DELETE';
                break;
            }
            if ($annotation instanceof \DevA7mad\Annotations\Annotations\Any) {
                $httpMethod = 'ANY';
                break;
            }
